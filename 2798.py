...
cnt, max_num = map(int,input().split())
    ...
    for j in range(1,cnt):
        ...
        for k in range(2, cnt):
            if num_list[i] == num_list[k] :
                continue
            elif num_list[j] == num_list[k] :
                continue
            #print(num_list[i], num_list[j], num_list[k])
            hap_list.append(num_list[i]+num_list[j]+num_list[k])

for i in range(len(hap_list)) :
   if hap_list[i] <= max_num :
      result.append(hap_list[i])
...
#             if res < hap[i] + hap[j] + hap[k] <= num2 :
#                 res = hap[i] + hap[j] + hap[k]
